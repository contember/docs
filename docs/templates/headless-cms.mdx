...
## How to custumise template

Now you are ready to start working. You can custumize our template as you want to. Below, you can find some examples of how to custumize the template.
...
Go to `headless-cms/api/model/ContentBlocks.ts` and add `'imageWithTextSection'` to `ContentBlockType` enum. We'll use `image` and `content` fields from `ContentBlock` to store our data.
...
#### 3. Add new block to Admin

Go to `headless-cms/admin/components/ContentBlocks.tsx` file and add [`<Block />`](/admin/components/blocks#block) component into [`<BlockRepeater />`](/admin/components/repeaters/#block-repeater) component. `<Block />` should contain all necessary fields for your new section. We want to have a image with text, so we'll add custom component `<ContentField />` (you can learn more in `/headless-cms/admin/components/ContentField.tsx`) for text and [`<ImageUploadField />`](/admin/components/upload#image-file-repeater) component for image.
...
Now you can check your new section in <a href="http://localhost:1480" target="_blank">Admin</a>.

#### 4. Add new section to Head

Go to `headless-cms/head/components/blocks.tsx` and add new react component called `ImageWithText` which will render content and image. Next, you'll need to add your new components to block map in `Blocks` component.
...
### Editting testimonials from one place
