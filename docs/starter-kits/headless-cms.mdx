...
Go to `headless-cms/admin/components/ContentBlocks.tsx` file and add [`<Block />`](/reference/admin/api/Blocks%20and%20repeaters/Block) component into [`<BlockRepeater />`](/reference/admin/api/Blocks%20and%20repeaters/BlockRepeater) component. `<Block />` should contain all necessary fields for your new section. We want to have an image with text, so we'll add a custom component `<ContentField />` (you can learn more in `/headless-cms/admin/components/ContentField.tsx`) for text and
[`<ImageUploadField/>`](/reference/admin/api/Uploads/ImageUploadField) component for the image.
