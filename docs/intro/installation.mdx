...
- macOS, Windows (including WSL), and Linux are supported. If you're using an ARM64 architecture, make sure to enable Use Rosetta for x86/amd64 emulation on Apple Silicon in your Docker Desktop settings/Features in development.
...
- `start-engine` runs `docker-compose up --detach` to start the Docker containers, followed by `npm run contember migrations:execute --yes` to execute database migrations.
...
- Minio local S3 provider at [http://localhost:1483](http://localhost:1483) (you can sign in with contembeer / contember credentials). Contember uses object storage to store files and images uploaded by your users.
...
<DocsCards>
  <DocsCard header="Quickstart" href="/intro/quickstart">
    <p>Build your first project from scratch.</p>
  </DocsCard>
</DocsCards>