...
* Datagrid: `HasOneSelectCell` and [`HasManySelectCell`](/reference/admin/api/Data%20grid/HasManySelectCell) [#87](https://github.com/contember/admin/pull/87)
...
- Move prop `actions` from the [`<TitleBar />`](/reference/admin/api/UI/TitleBar) component to page component (e.g. `<Page />` `<GenericPage />`, `<LayoutPage />`).
...
- We no longer support `<AltPersistButton />` component. Remove it or change it to [`<PersistButton />`](/reference/admin/api/Action%20buttons/PersistButton).

#### Tenant pages

- Change the [`<GenericPage />`](/reference/admin/api/Pages/GenericPage) component that wraps the tenant pages to `<Page />` component.
