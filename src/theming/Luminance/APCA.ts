...
const blkThrs = 0.022,
      ...
      loBoWthresh = loWoBthresh,
      loWoBfactor = 27.7847239587675,
      loBoWfactor = loWoBfactor,
      loWoBoffset = 0.027,
      loBoWoffset = loWoBoffset,
      loClip = 0.001,
      deltaYmin = 0.0005;
...
// ↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑
// Source: https://github.com/Myndex/SAPC-APCA/blob/master/src/JS/SAPC_0_98G_4g_minimal.js

const HEX_REGEX = new RegExp(/^#([0-9abcdef]{3}|[0-9abcdef]{6})$/i)

/**
 * Calculates the luminance of the color represented as a hexadecimal string
 *
 * For wider range of inputs use chroma.js
 *
 * @param hex Color as hex string
 * @returns Luminance of the color
 */
export function Y(hex: string) {
  const match = hex.match(HEX_REGEX)

  if (!match) {
    throw new Error("Passed string is not a color defined as hex string");
  }

  const hexString = match[1]
  const hexInteger = hexString.length === 6
    ? parseInt(hexString, 16)
    : parseInt(hexString.split('').map(t => t+t).join(''), 16)

  return sRGBtoY(hexInteger)
}

/**
 * Calculates the APCA contrast
 *
 * (A typed version of the official `APCAcontrast()`)
 * To calculate the Y (luminance) input use `Y()` method
 * or use chroma.js `luminance()` method.
 *
 * @param txtY Luminance of the foreground (text)
 * @param bgY Luminance of the background
 * @returns Contrast in APCA scale ±0-108 (approx)
 */
export function apcaContrast(txtY: number, bgY: number) {
  return APCAcontrast (txtY, bgY)
}

/**
 * Returns values in range of ±0-100
*/
export function APCAcontrastAsCents(foregroundY: number, backgroundY: number) {
  return (
    APCAcontrast(foregroundY, backgroundY) /
    (foregroundY > backgroundY ? 1.078847 : 1.060407)
  );
}
 ...
 * Reverses the method
 *
 * @param bgY Background luminance
 * @param inputContrast Expected contrast
 * @returns Foreground luminance
 */
export function reverseAPCAContrast(
  bgY: number,
  ...
  if (inputContrast > 0) {
    ...
    console.log(Math.pow(bgY, normBG) - SAPC / scaleBoW, normTXT);

    // SAPC = (Math.pow(bgY, revBG) - Math.pow(txtY, revTXT)) * scaleWoB;
    // SAPC / scaleWoB = Math.pow(bgY, revBG) - Math.pow(txtY, revTXT)
    // Math.pow(bgY, revBG) - (SAPC / scaleWoB) = Math.pow(txtY, revTXT)
    // Math.pow(txtY, revTXT) = Math.pow(bgY, revBG) - (SAPC / scaleWoB)
    return Math.pow(Math.pow(bgY, normBG) - SAPC / scaleBoW, 1 / normTXT);
  }
  // SAPC = (Math.pow(bgY, revBG) - Math.pow(txtY, revTXT)) * scaleWoB;

  console.log(
    Math.pow(bgY, revBG) - SAPC / scaleWoB,
    1 / revTXT,
    Math.pow(Math.pow(bgY, revBG) - SAPC / scaleWoB, 1 / revTXT)
  );
  return Math.pow(Math.pow(bgY, revBG) - SAPC / scaleWoB, 1 / revTXT);
}
